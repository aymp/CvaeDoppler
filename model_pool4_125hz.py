...
class Qy_x(nn.Module):    
    def __init__(self, y_dim=10):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=4,padding=2),   # 64x74 ⇒ 65x75
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(4)) # 65*16 ⇒ 16*18

        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=4, padding=2), # 16*18 ⇒ 17x19
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(4)) # 17x19 ⇒ 4x4
...
class Px_z(nn.Module):
    def __init__(self, z_dim=2):
        ...
        self.conv_d = nn.Sequential(
            nn.ConvTranspose2d(128, 64, kernel_size=(6,8), stride=4, padding=1), # 4x4 ⇒ 16x18
            nn.BatchNorm2d(64),
            nn.LeakyReLU(0.2),
            nn.ConvTranspose2d(64, 1, kernel_size=(6,8), stride=4, padding=1), # 16x18 ⇒ 64x74
            nn.Sigmoid()
        )
...
class Qy_x(nn.Module):    
    def __init__(self, y_dim=10):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=4,padding=2),   # 48x74 ⇒ 49x75
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(4)) # 49*75 ⇒ 12*18

        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=4, padding=2), # 12*18 ⇒ 13x19
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(4)) # 13x19 ⇒ 3x4

        self.fc =  nn.Sequential(
            #nn.Linear(-(-INPUT_HEIGHT//4) * -(-INPUT_WIDTH//4)* 128, 256),
            nn.Linear(3*4* 128, 256),
            nn.Dropout(p=0.4),
            nn.ReLU(),
            nn.Linear(256, y_dim),
            nn.Softmax(dim=1)
...
class Qy_x(nn.Module):    
    def __init__(self, y_dim=10):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(4,stride=2,padding=1)) # 65*75 ⇒ 32*37

        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=4, padding=2), # 32x37 ⇒ 33x38
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(4,stride=2,padding=1)) # 33x38 ⇒ 16x19
