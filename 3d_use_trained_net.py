...
# ----- TRAINED
    ...
    datatime_trained = datatime_array[t_array < Y_DIM]

    output_fp = open(DIR_OUT+"train.csv", 'w')
    for label_i, latent_i, datatime_i, label_pred in zip(t_trained, z_trained, datatime_trained, y_trained_pred):
...
# ----- ADDITIONAL SUBJECT
    ...
    t_add = t_array[t_array >= Y_DIM]
    datatime_add = datatime_array[t_array >= Y_DIM]

    output_fp = open(DIR_OUT+"additional.csv", 'w')
    for label_i, latent_i, datatime_i, label_pred in zip(t_add, z_add, datatime_add, y_add_pred):
        output_fp.write(str(label_i))
        output_fp.write(",")
        output_fp.write(str(label_pred))
        output_fp.write(",")
...
# ----- TEST DATA (TRAINED SUBJECT)
    ...
    output_fp = open(DIR_OUT+"test.csv", 'w')
    for label_i, latent_i, datatime_i, label_pred in zip(t_test, z_test, datatime_test, y_test_pred):
        output_fp.write(str(label_i))
        output_fp.write(",")
        output_fp.write(str(label_pred))
        output_fp.write(",")
