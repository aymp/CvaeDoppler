...
# ----- ここからループ開始してグラフ作りたい
    #for th_val in range(1,4,0.1): みたいなノリかな

    # 他人受入率(FAR; False Acceptance Rate)
    far_cnt = 0
    far_all = 0
    # 本人拒否率(FRR; False Recognition Rate)
    frr_cnt = 0
    frr_all = 0

    TH_VAL = 2 # TEMP あとで絶対ループ分に組み込む！

# ----- 訓練済み被験者（本人拒否率、FRR）
    # 今気づいたが、本人拒否して別の人と認識した場合、そっちでしきい値調べて他人受入率の考慮をしないとダメ？
    for i in range(Y_DIM):
        z_trained = z_pred[t_array < Y_DIM]
        y_trained_pred = y_pred[t_array < Y_DIM]
        t_trained = t_array[t_array < Y_DIM]

        z_trained = z_trained[t_trained == i]
        y_trained_pred = y_trained_pred[t_array == i]
        t_trained = t_trained[t_trained == i]

        frr_all += z_trained.shape[0] # 学習済みの人物なので、"本人"拒否率の分母にカウントする
        # [STEP1] 真のラベル noteq 予測ラベル ➡ 本人拒否
        frr_cnt += np.count_nonzero(t_trained != y_trained_pred) # FRR++

        # [STEP2] 本人拒否の場合、誤った予測ラベルの他人受入率の検討をしたい
        """TODO"""
        z_trained_mislabel = z_trained[np.where(t_trained!=y_trained_pred)] # 別ラベルだと認識したやつ
        y_trained_pred_mislabel = y_trained_pred[np.where(t_trained!=y_trained_pred)]
        t_trained_mislabel = t_trained[np.where(t_trained!=y_trained_pred)]
        
        for j in range(Y_DIM):
            if j != i:
                print('このコードは開発途中　FARとFRRを計算しようと思ったが複雑になり、表計算ソフトを用いたほうが適切と判断')

        z_trained = np.delete(z_trained,obj=np.where(t_trained!=y_trained_pred),axis=0) # STEP2との重複を嫌って削除
        # y_trained_pred = np.delete(y_trained_pred,obj=np.where(t_trained!=y_trained_pred),axis=0)
        # t_trained = np.delete(t_trained,obj=np.where(t_trained!=y_trained_pred),axis=0)

        # [STEP3] ラベル予測は正解だが、しきい値の条件を満たさない場合　➡　本人拒否
        trained_vectors = z_trained - z_trained_avg[i]
        trained_distances = np.linalg.norm(trained_vectors, ord=2, axis=1)
        frr_cnt += np.count_nonzero(trained_distances > TH_VAL)
...
# ----- Unknownな被験者（他人受入率）
    ...
    # ----- DISTANCE between each coordinate and trained-data's centroid
        if z_add.shape[0] != 0: # Unknownな被験者がそのラベルだと予測された場合のみ、カウントを行う(far_cnt)
            z_add_vector = z_add - z_trained_avg[i]
            z_add_dist = np.linalg.norm(z_add_vector, ord=2, axis=1)
            far_all += z_add_dist.shape[0]
            far_cnt += np.count_nonzero(z_add_dist<TH_VAL)
...
# ----- TEST DATA (TRAINED SUBJECT)
    # 訓練済みの人物は「本人拒否率」に関与する
    z_test = z_pred[t_array < Y_DIM]
    y_test_pred = y_pred[t_array < Y_DIM]
    t_test = t_array[t_array < Y_DIM]
    datatime_test = datatime_array[t_array < Y_DIM]
